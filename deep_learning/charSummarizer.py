...
#     lines = f.read().split('\n')
# for line in lines[: min(num_samples, len(lines) - 1)]: from translator
current = 0
maxData = 1000        #set max size of dataset
for story in stories:
    # input_text, target_text, _ = line.split('\t') from translator
    # We use "tab" as the "start sequence" character
    # for the targets, and "\n" as "end sequence" character.
    input_text = story['story']
    target_text = '\t' + story['highlights'] + '\n'
    print(input_text + " --- " + target_text)   #FIXME
...
model.fit([encoder_input_data, decoder_input_data], decoder_target_data,
          ...
          callbacks=cb_list)
# Save model
model.save('1000_data_char_model.h5')
...
#decoded sentence was the same for every single test input
